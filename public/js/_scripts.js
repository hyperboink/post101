$(function(){
	...
	win.scroll(function(){
		var height = $(document).height() - win.height(),
			...
			loadPost = $('.post-load'),
			hasMorePost = (postData.total > $('.post').length);
	...
	$('.notification-items').on('click', function(){
		//$('.');
	});
	...
	function renderNotification(notification){
		return '<a class="dropdown-item '+(!notification.read_at ? 'unread': 'read')+'" href="'+notificationUpdate(notification).link+'">\
				<span class="notification-name">'+notification.data.name+'</span> '+notificationUpdate(notification).msg+'\
			</a>'
	}
	...
	function notificationUpdate(notification){
		...
		switch(notification.data.action){
			case 'follower':
				notificationUpdate.link = '/profile/'+notification.data.name+'?read='+notification.id;
				notificationUpdate.msg = 'is now following you.';
				break;
			case 'poster':
				notificationUpdate.msg = 'has a new post.';
				break;
			case 'commenter':
				notificationUpdate.msg = 'commented on your post.';
				break;
			case 'liker':
				notificationUpdate.msg = 'liked your post.'
		}
